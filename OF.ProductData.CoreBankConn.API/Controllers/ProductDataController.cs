...
public class ProductDataController : ControllerBase
{
    ...
    public ProductDataController(IProductDataService EnquiryService, ICoreBankRepository repository, IOptions<ServiceParams> serviceParams, ProductLogger enquiryLogger, WarmUpLogger warmUpLogger)
    {
    ...
    public async Task<IActionResult> GetProductData(CbsProductRequest coreBankProductRequest)
    {
        var coreBankproductResponse = new ApiResult<CbsProductResponse>
        {
            Success = false,
            Code = "400",
        ...
        try
        {
            ApiResult<CbsProductResponse>? result = null;
            ...
            {
                var coreBankproductdataResponse = new CbsProductResponse
                {
        ...
        {
            _Logger.Error(ex);
            return BadRequest(ApiResultFactory.Failure<CbsProductResponse>("An unexpected error occurred."));
        }
    }

    private async Task PrepareEnquiryRequest(CbsProductRequest coreBankProductRequest)
    {
        try
        {
